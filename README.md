...
```
Binance WS ──► ingest ──┬──► klines  (미확정 봉도 upsert)
                        ├──► trades  (버퍼 → 배치 삽입)
                        └──► 이벤트 ──┬──► metrics 캐시
                                      └──► realtime (SSE) ──► 브라우저

Binance REST ──► backfill ──► klines  (갭 구간 채우기)
                    ▲
                    └── 갭 스캔(60초) · WS 재연결 · 수동 트리거

PostgreSQL ──► api (candles / metrics / ops) ──► 브라우저
```

**쓰기 경로가 둘(실시간·백필)이고 읽기 경로가 하나**입니다.
두 쓰기 경로가 같은 테이블을 건드리므로 **멱등성과 순서 보호가 설계의 중심**이 됐습니다
(4.3 데이터 무결성 참조).
...
| 모듈 | 책임 | 분리 이유 |
|---|---|---|
| `binance` | 거래소 통신만 (REST · WS · 레이트리밋) | 거래소 API 변경의 영향을 이 안에 격리 |
| `ingest` | 실시간 수집 | WS 수명주기와 배치 버퍼가 독립적 관심사 |
| `backfill` | 갭 탐지 · 복구 | 실시간과 별개 트리거(주기 / 재연결 / 수동)로 동작 |
| `metrics` | 지표 집계 · 캐시 | 무거운 SQL과 캐시 정책을 한곳에 |
| `api` · `realtime` | HTTP 표면 · SSE | 도메인 로직이 전송 방식을 모르게 함 |
| `db` | 스키마 · 리포지토리 | 쓰기 규칙(upsert 가드, 청크 분할)을 한곳에서 강제 |

모듈 간 결합은 두 가지로 끊었습니다.

- **포트 인터페이스** (`common/ports/`) — API·SSE 레이어는 `MetricsService` 구현을 직접 import하지 않고
  `METRICS_PORT` 토큰으로 주입받습니다. 각 서비스는 `implements MetricsPort`를 명시해
  **계약이 어긋나면 런타임이 아니라 컴파일에서 멈춥니다.**
- **이벤트 버스** (`EventEmitter2`) — `ingest`가 봉을 저장한 뒤 이벤트를 발행하면
  지표 캐시와 SSE가 각각 구독합니다. 구독자가 늘어도 수집기를 고치지 않습니다.
...
| `trades` | 개별 체결. PK `(symbol, trade_id)` — 거래소가 부여한 시퀀스라 중복이 자연 제거됨 |
| `ingest_state` | 스트림별 마지막 수신 지점. `/ops/health`의 수집 지연 표시에 사용 |
| `backfill_jobs` | 백필 이력. **복구가 실제로 돌았다는 증거** |
| `collector_events` | 연결/끊김/갭탐지/레이트리밋 로그 |

앞의 둘이 시장 데이터, 뒤의 셋이 운영 메타데이터입니다.
**운영 정보를 로그 파일이 아니라 테이블로 만든 것이 의도적인 선택**입니다 —
"백필이 실제로 돌았다"를 대시보드에서 조회해 보여주려면 쿼리가 가능해야 했습니다.

### 설계 근거

**PK를 자연키로 잡았습니다.** surrogate `id`를 두지 않았습니다.

- `klines` = `(symbol, interval, open_time)` — 같은 심볼·인터벌·시각의 봉은 논리적으로 하나만
  존재해야 합니다. 그 제약을 PK로 직접 표현하면 **중복 방지가 자동으로 보장**되고,
  `ON CONFLICT`가 PK를 그대로 쓸 수 있어 idempotent upsert가 자연스럽습니다.
  PK 인덱스가 곧 시계열 조회 인덱스가 되는 이득도 있습니다.
- `trades` = `(symbol, trade_id)` — `trade_id`는 Binance가 부여하는 심볼별 증가 시퀀스입니다.
  거래소가 이미 고유 식별자를 주므로 별도 id를 만들지 않았고, 덕분에 **중복 수신을 DB가 거부**합니다.
  `trade_id`는 이미 수십억대라 `bigint`로 다룹니다.

**금액·수량은 `numeric`입니다.** `double precision`이 아닙니다.

```sql
open   numeric(24, 8)   -- 가격  (정수부 16 · 소수부 8)
volume numeric(32, 8)   -- 수량·금액 (합계가 들어갈 수 있어 정수부를 더 크게)
```

IEEE754 부동소수는 십진 소수를 정확히 표현하지 못합니다(`0.1 + 0.2 !== 0.3`).
가격 하나는 오차가 미미하지만 **거래대금을 24시간 누적하면 눈에 보이는 수준으로 쌓입니다.**
소수 8자리는 Binance가 쓰는 최대 자릿수에 맞췄습니다.
애플리케이션은 거래소가 준 문자열을 `parseFloat` 없이 그대로 넣고, 집계는 SQL에서 끝냅니다.

**모든 시각은 `timestamptz`입니다.** `timestamp without time zone`은 '벽시계 시각'이라
어느 지역인지 정보가 없어 배포 환경이 바뀌면 조용히 어긋납니다.
Binance가 epoch milliseconds(절대시각)를 주므로 `timestamptz`가 정확한 대응이고,
컨테이너도 `TZ=UTC`로 고정했습니다.

**인덱스는 조회 패턴에서 역산했습니다.** 전부 "최근 것부터" 보는 패턴이라 DESC입니다.
`trades`는 PK가 `trade_id` 기준이어서 시각 범위 조회에 쓸 수 없어 `(symbol, trade_time DESC)`를
별도로 걸었습니다.

**파생 인터벌은 저장하지 않습니다.** `5m`/`15m`/`1h`는 조회 시 1분봉에서 SQL로 계산합니다.
저장하면 원본과 어긋날 여지가 생기고(백필로 1분봉이 수정되면 파생분도 다시 계산해야 함),
1분봉이 원본이고 나머지는 그것의 함수라는 관계가 흐려집니다.
버킷 내 `open`/`close`는 min/max가 아니라 **시간 순서 기준 첫/마지막**으로 계산합니다
(자세한 규칙은 [docs/METRICS.md](docs/METRICS.md) 3절).

**하지 않은 것들**: 심볼 정규화(심볼이 2개뿐이고 환경변수로 관리 — JOIN 비용만 늘어남),
파티셔닝(현재 약 8,600행으로 오버엔지니어링. 확장 시 `open_time` range 파티셔닝),
`is_closed` 컬럼(단조 증가 성질로 제약을 걸어 대체 — 4.3 참조).
...
235개 통과. **DB·네트워크 없이 도는 순수 로직**만 테스트합니다 — 의도적인 선택입니다.

이 프로젝트에서 틀리면 치명적인 로직은 대부분 순수 계산입니다 —
갭 탐지(시퀀스 차집합·구간 병합), 페이지네이션 커서 전진, 레이트리미터 예산,
지표 수식, 배치 청크 크기. 그래서 **그것들을 순수 함수로 분리되도록 설계**했습니다.
예를 들어 `GapDetector`는 DB 조회를 리포지토리에 위임하고,
시퀀스 생성·차집합·구간 병합은 `gap-math.ts`의 순수 함수로 빼서 DB 없이 전 경계를 검증합니다.
...
- 기동 백필 판단: 실시간 스트림이 방금 넣은 봉 때문에 최초 백필을 건너뛰지 않는지 (회귀 방지)
...
- **DB 통합 테스트 없음** — upsert 가드와 집계 SQL은 실제 DB가 있어야 검증됩니다.
  대신 생성 SQL을 출력해 확인하고 실제 기동해 결과를 대조했습니다
  (예: 1시간봉의 "앞 봉 종가 = 뒤 봉 시가" 연속성). 제대로 하려면 Testcontainers가 맞습니다.
