...
│   │   ├── metrics/            지표 집계 · 캔들 파생 · 스냅샷 캐시
│   │   ├── api/                REST 컨트롤러 (candles / metrics / ops)
│   │   ├── realtime/           SSE 스트림
│   │   ├── db/                 스키마 · 마이그레이션 · 리포지토리
...
| reason | 트리거 | 동작 |
|---|---|---|
| `bootstrap` | 기동 시 DB가 비어 있음 | `BOOTSTRAP_BACKFILL_DAYS`만큼 과거 적재 |
| `gap_recovery` | 기동 직후 + 60초 주기 | 마지막 저장 지점 이후 = 다운타임 갭. 상시 누락도 함께 탐지 |
...
231개 통과. DB·네트워크 없이 도는 순수 로직 테스트에 집중했습니다.
